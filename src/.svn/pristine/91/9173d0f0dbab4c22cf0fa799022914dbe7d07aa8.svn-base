 ...
 * 
 */
public class TimeWin extends Window {
	
	private int width = 295;
	private int height = 194;
	...
	public TimeWin(ListGrid table,String customer_id,int row,String top,String left, String type){
		this.table = table;
		this.row = row;
		this.top = top;
		this.left = left;
	...
	public TimeWin(ListGrid table,int row,String top,String left,String type){
		this.table = table;
		this.row = row;
		this.top = top;
		this.left = left;
	...
	public Window getViewPanel() {
		...
		FULL_INDEX.setDefaultValue(ObjUtil.ifObjNull(table.getEditedCell(row, "FORMULA_DESCR"),"").toString());
        ...
        if(type.equals(StaticRef.SHPM_NO)) {
        	Util.initComboValue(combo, "USER_TAB_COLUMNS", "COLUMN_NAME", "COLUMN_NAME", " where TABLE_NAME = 'V_TIME_SHPM'", "");
        }
        else {
        	Util.initComboValue(combo, "USER_TAB_COLUMNS", "COLUMN_NAME", "COLUMN_NAME", " where TABLE_NAME = 'V_TIME_ODR'", "");
        }
        combo.addChangedHandler(new ChangedHandler() {
			...
			public void onChanged(ChangedEvent event) {
				if(event.getValue() != null) {
					...
					searchPanel.setValue("FULL_INDEX", FULL_INDEX.getDisplayValue() + cur_value);
					//table.setEditValue(row, "FORMULA", FULL_INDEX.getDisplayValue() + event.getValue().toString());
					table.setEditValue(row, "FORMULA", FULL_INDEX.getDisplayValue());
					combo.setValue("");
				}
			}
