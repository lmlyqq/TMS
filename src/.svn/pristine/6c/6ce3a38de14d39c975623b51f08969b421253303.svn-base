...
import com.rd.client.action.settlement.DeleteReceRuleAction;
import com.rd.client.action.settlement.NewReceDiscountAction;
import com.rd.client.action.settlement.NewReceRuleActiom;
import com.rd.client.action.settlement.SaveReceRuleAction;
...
//		v_lay.setVisible(false);
	...
	private void createListField() {
		...
		ListGridField WHR_UDF1 = new ListGridField("WHR_UDF1","条件1",70);
		ListGridField WHR_UDF2 = new ListGridField("WHR_UDF2","条件2",70);
		ListGridField WHR_UDF3 = new ListGridField("WHR_UDF3","条件3",70);
		ListGridField WHR_UDF4 = new ListGridField("WHR_UDF4","条件4",70);
		ListGridField WHR_UDF5 = new ListGridField("WHR_UDF5","条件5",70);
		...
		table.setFields(TFF_NAME,FEE_NAME,FEE_BASE_NAME,GRADE_BASE,WHR_UDF1,WHR_UDF2,WHR_UDF3,WHR_UDF4,WHR_UDF5,LOWER_LMT_OPRT,
	...
	private SectionStack createFeeInfo(){
		...
		Util.initComboValue(TFF_NAME,"TARIFF_HEADER","ID","TFF_NAME","  where tff_typ = '42666CA2DE904F6687FC172138CF3E51' and COM_FLAG = 'N' and EXEC_ORG_ID IN (SELECT ID From BAS_ORG Where id ='"+ord_id+"'or ORG_INDEX Like '%,"+ord_id+",%') ","");
...
//		vm.addMember(basInfo);
	...
	public void createBtnWidget(ToolStrip strip) {
		...
		newB = createBtn(StaticRef.CREATE_BTN,SettPrivRef.ReceRule_P0_01);
		newB.addClickHandler(new NewReceRuleActiom(vm, cache_map,this));
		
		savB = createBtn(StaticRef.SAVE_BTN,SettPrivRef.ReceRule_P0_02);
		savB.addClickHandler(new SaveReceRuleAction(table,vm,check_map,this));
		
		delB = createBtn(StaticRef.DELETE_BTN,SettPrivRef.ReceRule_P0_03);
		delB.addClickHandler(new DeleteReceRuleAction(table, vm));
		
		canB = createBtn(StaticRef.CANCEL_BTN,SettPrivRef.ReceRule_P0_04);
		...
		copyButton = createBtn(StaticRef.COPY_BTN,SettPrivRef.ReceRule_P0_09);
		...
		add_detail_map.put(SettPrivRef.ReceRule_P0_01, newB);
		add_detail_map.put(SettPrivRef.ReceRule_P0_09, copyButton);
		del_detail_map.put(SettPrivRef.ReceRule_P0_03, delB);
		sav_detail_map.put(SettPrivRef.ReceRule_P0_02, savB);
		sav_detail_map.put(SettPrivRef.ReceRule_P0_04, canB);
	...
	private ToolStrip createDiscountBtn(){
		...
		newD = createBtn(StaticRef.CREATE_BTN,SettPrivRef.ReceRule_P0_05);
		newD.addClickHandler(new NewReceDiscountAction(discountTable, null, this));
		...
		savD = createBtn(StaticRef.SAVE_BTN,SettPrivRef.ReceRule_P0_06);
		...
		delD = createBtn(StaticRef.DELETE_BTN,SettPrivRef.ReceRule_P0_07);
		delD.addClickHandler(new DeleteAction(discountTable));
		
		canD = createBtn(StaticRef.CANCEL_BTN,SettPrivRef.ReceRule_P0_08);
		...
		add_discount_map.put(SettPrivRef.ReceRule_P0_05, newD);
		del_discount_map.put(SettPrivRef.ReceRule_P0_07, delD);
		sav_discount_map.put(SettPrivRef.ReceRule_P0_06, savD);
		sav_discount_map.put(SettPrivRef.ReceRule_P0_08, canD);
	...
	protected DynamicForm createSearchForm(DynamicForm form) {
		...
		SGText WHR_UDF1 = new SGText("WHR_UDF1","条件1",true);
		
		SGText WHR_UDF2 = new SGText("WHR_UDF2","条件2");
		
		SGText WHR_UDF3 = new SGText("WHR_UDF3","条件3");
		
		SGText WHR_UDF4 = new SGText("WHR_UDF4","条件4",true);
		
		SGText WHR_UDF5 = new SGText("WHR_UDF5","条件5");
		
		SGText WHR_UDF6 = new SGText("WHR_UDF6","条件6");
		
		form.setItems(TFF_TYP,TFF_NAME, FEE_ID,WHR_UDF1,WHR_UDF2,WHR_UDF3,WHR_UDF4,WHR_UDF5,WHR_UDF6,COM_FLAG);
			...
			public void onSuccess(ArrayList<HashMap<String, String>> result) {
				...
				if(result != null && result.size() > 0) {					
					...
					for(int j = 0; j < result.size(); j++){
						...
						if(maps != null&&maps.size()>0&&maps.size()>j) {
							leftCombos[j].setValue(ObjUtil.ifNull(maps.get(j).get("LEFT_BRKT"),""));
						}
						leftCombos[j].setShowTitle(false);
						leftCombos[j].setVisible(true);
	...
	public static void initArea(final ComboBoxItem area_name,final TextItem area_id,final String defaultValue){
		...
		area_name.setPickListWidth(300);
