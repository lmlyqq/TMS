...
//						view.feeTable.selectRecord(record);	
						...
						Util.db_async.getRecord("TOT_AMOUNT,ADJ_AMOUNT", "TRANS_LOAD_HEADER", " where LOAD_NO='"+view.loadTable.getSelectedRecord().getAttribute("LOAD_NO")+"'", null, new AsyncCallback<ArrayList<HashMap<String,String>>>() {
							...
							public void onSuccess(ArrayList<HashMap<String, String>> result) {
								if(result!=null&&result.size()>0){
									view.loadTable.getSelectedRecord().setAttribute("TOT_AMOUNT", result.get(0).get("TOT_AMOUNT"));
									view.loadTable.getSelectedRecord().setAttribute("ADJ_AMOUNT", result.get(0).get("ADJ_AMOUNT"));
									view.loadTable.getSelectedRecord().setAttribute("AUDIT_STAT_NAME", "待审批");
...
//						view.feeTable.redraw();
						...
						criteria.addCriteria("LOAD_NO", view.loadTable.getSelectedRecord().getAttribute("LOAD_NO"));
						view.feeTable.fetchData(criteria);
						Util.db_async.getRecord("TOT_AMOUNT", "TRANS_LOAD_HEADER", " where LOAD_NO='"+view.loadTable.getSelectedRecord().getAttribute("LOAD_NO")+"'", null, new AsyncCallback<ArrayList<HashMap<String,String>>>() {
