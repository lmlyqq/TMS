...
//		  ListGridField RATIO=new ListGridField("RATIO",Util.TI18N.RATION(),50);
	...
	public void createBtnWidget(ToolStrip toolStrip) {
        ...
        IButton searchButton = createBtn(StaticRef.FETCH_BTN,BasPrivRef.SKU_P0);
...
//        toolStrip.addSeparator();
        ...
        IButton inputButton = createBtn(StaticRef.IMPORT_BTN, BasPrivRef.SKU_P0_06);
		inputButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				if(uploadWin == null){
					...
					uploadWin = new UploadFileWin().getViewPanel("sku.xls","TMP_SKU_IMPORT","SP_IMPORT_SKU",table,pageForm);
				}else{
					uploadWin.show();
				}
			}
...
//		Util.initCustComboValue(customer_id, "", "",LoginCache.getDefCustomer().get("CUSTOMER_ID"));
        ...
        form.setItems(CUSTOMER_ID,CUSTOMER_NAME,sku_cls,EXEC_ORG_ID_NAME,C_ORG_FLAG,txt_enable,EXEC_ORG_ID);
