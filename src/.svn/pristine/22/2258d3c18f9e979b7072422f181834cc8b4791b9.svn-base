...
public class SaveSFAddrAction implements ClickHandler {
	...
	public SaveSFAddrAction(ListGrid p_table, ValuesManager p_form, HashMap<String, String> p_map, Map<String, String > p_corr) {
		table = p_table;
		form = p_form;  
		map = p_map;
		id_name = p_corr;
	}
	public SaveSFAddrAction(ListGrid p_table, ValuesManager p_form, HashMap<String, String> p_map) {
		table = p_table;
		form = p_form;  
		map = p_map;
	}
	
	public SaveSFAddrAction(ListGrid p_table, ValuesManager p_form, HashMap<String, String> p_map,DynamicForm mainForm,SGForm view) {
		table = p_table;
		form = p_form;  
		map = p_map;
...
//		id_name = p_corr;
	...
	public void onClick(ClickEvent event) {
		...
		if(table!=null&&table.getSelectedRecord()!=null){
			SF_ADDR_CODE=table.getSelectedRecord().getAttribute("ADDR_CODE");
		}
		...
		Util.async.doUpdate(logList, sqlList, new AsyncCallback<String>() {
			...
			public void onSuccess(String result) {
				if(result.equals(StaticRef.SUCCESS_CODE)) {
					...
					sf.append("update TRANS_SSS_ADDR");
					sf.append(" set SF_ADDR_CODE='"+mainForm.getItem("ADDR_CODE").getDisplayValue()+"'");
