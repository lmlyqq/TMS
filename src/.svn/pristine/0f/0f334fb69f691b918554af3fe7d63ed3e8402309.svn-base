...
import com.rd.client.ds.tms.SuplrFeeDS;
...
public class BmsSuplrFeeView extends SGForm implements PanelFactory {
	...
	public Canvas getViewPanel() {
		...
		loadDs = SuplrFeeDS.getInstance("BMS_LOAD_HEADER","BMS_LOAD_HEADER");
			...
			public void onClick(ClickEvent event) {
				if(loadTable.getSelectedRecord()!=null){
					...
					for(int i=0;i<records.length;i++){
						...
						if(record.getAttributeAsBoolean("USE_FLAG")) {
							if(record.getAttributeAsString("AUDIT_STAT_NAME").equals("审批完成")){
								SC.say(""+record.getAttributeAsString("LOAD_NO")+"调度单号审批完成");
								return;
							}
							else {
								bolExists = true;
					...
					if(!bolExists) {
						MSGUtil.sayError("审核的记录中包含审批完成的调度单!");
						return;
					}
					getCheckWin().show();
				}
...
//		disAudit(false, true);	
		...
		IButton createInitButton = createUDFBtn("生成对账单",StaticRef.ICON_SAVE,SettPrivRef.SUPLRFEE_P0_09);
		createInitButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				 ...
				 if(records!=null&&records.length>0){
					 for(int i=0;i<records.length;i++){
						 if(records[i].getAttribute("USE_FLAG")!=null&&records[i].getAttribute("USE_FLAG").equals("true")){
							 list.add(records[i]);	 
						 }						 
					 } 
				 ...
				 if(list.size()>0){ 
					 String suplr_id=list.get(0).getAttribute("SUPLR_ID");
					 for(int i=0;i<list.size();i++){
						 if((list.get(i).getAttribute("INIT_FLAG")!=null)&&("true").equals(list.get(i).getAttribute("INIT_FLAG"))){
							 MSGUtil.sayError("调度单【"+list.get(i).getAttribute("LOAD_NO")+"】已生成月初账单,不允许重复生成!");
					         return;
						 }
						 if(!suplr_id.equals(list.get(i).getAttribute("SUPLR_ID"))){
							 MSGUtil.sayError("勾选的调度单存在多个承运商，无法生成对账单!");
							 return;
						 }
					 }
					 InitWin(suplr_id,list).show();
				 }else{
					 MSGUtil.sayError("请勾选记录!");
					 return;
				 } 		 
			}
		});
		...
		toolStrip.setMembers(searchButton,confirmAudit,createInitButton);
	}
...
//		CUSTOMER_NAME.setColSpan(2);
		...
		final ComboBoxItem SUPLR_ID_NAME = new ComboBoxItem("SUPLR_ID_NAME", "承运商");
...
//		AUDIT_STAT.setValue("20");
		...
		form.setItems(CUSTOMER_ID,CUSTOMER_NAME,LOAD_NO,CUSTOM_ODR_NO_NAME,SHPM_NO,AUDIT_STAT,
				STATUS_FROM,STATUS_TO,SUPLR_ID,SUPLR_ID_NAME,PLATE_NO,START_AREA_ID,
				...
				AUDIT_TIME_FROM,AUDIT_TIME_TO,LOAD_NAME,UNLOAD_NAME,C_ORG_FLAG);
		return form;
	}
	@Override
	...
	private void createSettAsLoad(){
		loadTable=new SGTable(loadDs, "100%", "100%", false, false, false) {
			...
			protected Canvas getExpansionComponent(final ListGridRecord record) {    				  
               ...
               // lstTable.setCanEdit(false);
		        ...
		        ListGridField TOT_QTNY = new ListGridField("TOT_QNTY","数量", 60);
		...
		ListGridField TOT_AMOUNT = new ListGridField("TOT_AMOUNT","总成本", 80);
		...
		UDF2.setCanEdit(false);
		UDF2.setShowHover(true);
		ListGridField UDF3 = new ListGridField("UDF3","起止地", 130);
		UDF3.setCanEdit(false);
		UDF3.setShowHover(true);
		
		loadTable.setFields(USE_FLAG,LOAD_NO,STATUS,SUPLR_NAME,PLATE_NO,VEHICLE_TYP_ID,START_AREA_NAME,END_AREA_NAME,DRIVER1,TOT_GROSS_W,TOT_VOL,TOT_QNTY,AUDIT_STAT,AUDIT_NOTES,TOT_AMOUNT,ADJ_AMOUNT,UDF2,UDF3);
	...
	public Window getCheckWin() {
	    ...
	    saveButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
					...
					final ListGridRecord[] records = loadTable.getRecords();
					final List<ListGridRecord> recordList=new ArrayList<ListGridRecord>();
					...
					str.append("'");
					str.append(records[0].getAttribute("LOAD_NO"));		
					str.append("'");
					for(int i=0;i<records.length;i++){
						ListGridRecord record = records[i];		
						if(record.getAttributeAsBoolean("USE_FLAG")){
							list.put(Integer.toString(i+1),record.getAttributeAsString("LOAD_NO"));
							recordList.add(record);
						}
						if(i>0){
							str.append(",");
							str.append("'");
							str.append(records[i].getAttribute("LOAD_NO"));
							str.append("'");
						}
					}
						...
						public void onSuccess(String result) {
							if(result.substring(0,2).equals("00")){
								...
								for(int i=0;i<records.length;i++){
									...
									if(rec.getAttributeAsBoolean("USE_FLAG")){
										if(status.equals("同意")){
								...
								Util.db_async.getRecord("TOT_AMOUNT,ADJ_AMOUNT,LOAD_NO", "BMS_LOAD_HEADER", " where LOAD_NO in("+str+")", null, new AsyncCallback<ArrayList<HashMap<String,String>>>() {
										...
										if(result!=null&&result.size()>0){
											for(int i=0;i<result.size();i++){
												for(int j=0;j<recordList.size();j++){
													if(result.get(i).get("LOAD_NO").equals(recordList.get(j).getAttribute("LOAD_NO"))){
														recordList.get(j).setAttribute("TOT_AMOUNT", result.get(i).get("TOT_AMOUNT"));
														recordList.get(j).setAttribute("ADJ_AMOUNT", result.get(i).get("ADJ_AMOUNT"));
													}
												}
											}
										}
	...
	public Window InitWin(final String suplr_id,final List<ListGridRecord> list) {
		final Window win=new Window();
		SGPanel checkForm=new SGPanel();	
		
		final SGDate END_LOAD_TIME_FROM = new SGDate("END_LOAD_TIME_FROM", Util.TI18N.END_LOAD_TIME()+" 从");
		
		final SGDate END_LOAD_TIME_TO = new SGDate("END_LOAD_TIME_TO", "到");		
		
		checkForm.setItems(END_LOAD_TIME_FROM,END_LOAD_TIME_TO);
		checkForm.setIsGroup(true);  
		checkForm.setGroupTitle("请选择对账单的时间范围");
		checkForm.setMargin(3);
		checkForm.setPadding(20);
		checkForm.setHeight("70%");
		checkForm.setWidth("99%");
	    ...
	    saveButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				 //取发运时间
				final String load_time1=ObjUtil.ifObjNull(END_LOAD_TIME_FROM.getValue(), "").toString();
				final String load_time2=ObjUtil.ifObjNull(END_LOAD_TIME_TO.getValue(), "").toString();
				if(ObjUtil.isNotNull(load_time1)&&ObjUtil.isNotNull(load_time2)){
					SC.confirm("是否确定生成对账单？", new BooleanCallback() {
						public void execute(Boolean value) {
		                    if (value != null && value) {
		                    	HashMap<String, Object> listmap = new HashMap<String, Object>(); 
		                    	listmap.put("1",suplr_id);
		                    	System.out.println(suplr_id);
		            			listmap.put("2",load_time1);
		            			System.out.println(load_time1);
		            			listmap.put("3",load_time2);
		            			System.out.println(load_time2);
		            			final String json = Util.mapToJson(listmap);
		            			Util.async.execProcedure(json, "BMS_TIMER_CREATEPAY(?,?,?,?)",new AsyncCallback<String>() {
									
									@Override
									public void onSuccess(String result) {
										if(result.substring(0, 2).equals(StaticRef.SUCCESS_CODE)){
											MSGUtil.showOperSuccess();
											END_LOAD_TIME_FROM.clearValue();
											END_LOAD_TIME_TO.clearValue();
											if(list!=null&&list.size()>0){
												for(int i=0;i<list.size();i++){
													list.get(i).setAttribute("INIT_FLAG", true);
												}
											}
											win.hide();
										}
	...
	private void initSuplr(final ComboBoxItem suplr_name,final TextItem suplr_id){
		DataSource supDS = BasVehSuplierDS.getInstall("BAS_VEH_SUPPLIER2", "BAS_SUPPLIER");
