...
import com.rd.client.action.settlement.NewSuplrTariffAction;
import com.rd.client.action.settlement.SaveSuplrTaffRulAction;
import com.rd.client.action.settlement.tariff.NewSuplrTariffCondAction;
import com.rd.client.action.settlement.tariff.NewSuplrTariffRuleAction;
import com.rd.client.action.settlement.tariff.NewTariffSuplrDiscountAction;
...
import com.rd.client.ds.base.SUPPLIERDS;
 ...
 */
@ClassForNameAble
public class SuplrRateManagerView extends SGForm implements PanelFactory {
	...
	/*public SuplrRateManagerView(String id) {
		super(id);
	}*/
...
//		SIGN_ORG_ID_NAME.setTitle(ColorUtil.getRedTitle(Util.TI18N.SIGN_ORG_ID()));
	...
	private SectionStack createFeeInfo(){
		...
		final SGText FEE_BASE_NAME = new SGText("FEE_BASE_NAME",Util.TI18N.FEE_BASE());
							...
							if(result!=null&&result.size()>0){
								for(int i=0;i<result.size();i++){
									...
									for(int j=0;j<records.length;j++){
										System.out.println(records[j].getAttribute("ID")+"   "+map.get("FACTOR_ID"));
...
//		FEE_FIELD.setCanEdit(false);
		...
		Util.db_async.getRecord("ID,FEE_FACTOR", "TARIFF_FACTOR", " where FEE_TYPE='55CDDBECF2BF49D1A451F09A263F3F47' ", null, new AsyncCallback<ArrayList<HashMap<String,String>>>() {
		...
		feeTable.setFields(START_AREA_ID_NAME,END_AREA_ID_NAME,BIZ_TYP,FEE_NAME,FEE_BASE_NAME,GRADE_BASE,LOWER_LMT_OPRT,
	...
	private void createbottoInfo() {
		...
		OPER_OBJ.addChangedHandler(new com.smartgwt.client.widgets.grid.events.ChangedHandler() {
			...
			public void onChanged(
				...
				String biz_object=record.getAttribute("OPER_OBJ");
				Util.initComboValue(OPER_ATTR, "TARIFF_FACTOR", "ID", "FEE_FACTOR", " WHERE FEE_TYPE='55CDDBECF2BF49D1A451F09A263F3F47' and BIZ_OBJECT='"+biz_object+"' ", "");
	...
	public void createBtnWidget(ToolStrip toolStrip) {
		...
		// 新增按钮
		newButton = createBtn(StaticRef.CREATE_BTN,SettPrivRef.SUPLR_RATE_P1_01);
		newButton.addClickHandler(new NewSuplrTariffAction(basInfo, cache_map,this));
		
		HashMap<String, String> map = new HashMap<String, String>();
		map.put("OBJECT_ID", "OBJECT_NAME");
		
		// 保存按钮
		saveButton = createBtn(StaticRef.SAVE_BTN,SettPrivRef.SUPLR_RATE_P1_02);
		saveButton.addClickHandler(new SaveFormAction(table, basInfo, check_map, this,map));

		// 删除按钮
		delButton = createBtn(StaticRef.DELETE_BTN,SettPrivRef.SUPLR_RATE_P1_03);
		delButton.addClickHandler(new DeleteFormAction(table, basInfo));

		// 取消按钮
		canButton = createBtn(StaticRef.CANCEL_BTN,SettPrivRef.SUPLR_RATE_P1_04);
		canButton.addClickHandler(new CancelTariffAction(table, basInfo, this));
		
		
		IButton jfSaveButton=createBtn("保存设置",SettPrivRef.SUPLR_RATE_P1_14);
					...
					if(records!=null&&records.length>0){
						...
						for(int i=0;i<records.length;i++){
							if(records[i].getAttribute("YN_FLAG").equals("true"))
							{
								...
								sf = new StringBuffer();
								sf.append("insert into TARIFF_HEADER_FACTOR(TFF_ID,FACTOR_ID,SHOW_SEQ) VALUES ('"+TFF_ID+"','"+FACTOR_ID+"',");
								if(SHOW_SEQ==null){
									sf.append(records[i].getAttribute("RecordNO")+")");
								}else{
									sf.append(SHOW_SEQ+")");
								}
		...
		add_map.put(SettPrivRef.SUPLR_RATE_P1_01, newButton);
		del_map.put(SettPrivRef.SUPLR_RATE_P1_02, delButton);
		save_map.put(SettPrivRef.SUPLR_RATE_P1_03, saveButton);
		save_map.put(SettPrivRef.SUPLR_RATE_P1_04, canButton);
	...
	private ToolStrip createFeeBtn(){
		...
		copyButton = createBtn(StaticRef.COPY_BTN,SettPrivRef.SUPLR_RATE_P1_05);
		copyButton.addClickHandler(new CopyAction(basInfo2, this));
		
		newB = createBtn(StaticRef.CREATE_BTN,SettPrivRef.SUPLR_RATE_P1_06);
		newB.addClickHandler(new NewSuplrTariffRuleAction(vm, fee_cache_map,getThis()));
...
//		map.put("FEE_TYP", "FEE_TYP_NAME");
		...
		savB = createBtn(StaticRef.SAVE_BTN,SettPrivRef.SUPLR_RATE_P1_07);
		savB.addClickHandler(new SaveSuplrTaffRulAction(feeTable, vm, detail_ck_map,this));
		
		delB = createBtn(StaticRef.DELETE_BTN,SettPrivRef.SUPLR_RATE_P1_08);
		delB.addClickHandler(new DeleteFormAction(feeTable, basInfo2));
		
		canB = createBtn(StaticRef.CANCEL_BTN,SettPrivRef.SUPLR_RATE_P1_09);
		...
		add_detail_map.put(SettPrivRef.SUPLR_RATE_P1_06, newB);
		add_detail_map.put(SettPrivRef.SUPLR_RATE_P1_05, copyButton);
		del_detail_map.put(SettPrivRef.SUPLR_RATE_P1_08, delB);
		sav_detail_map.put(SettPrivRef.SUPLR_RATE_P1_07, savB);
		sav_detail_map.put(SettPrivRef.SUPLR_RATE_P1_09, canB);
	...
	private ToolStrip createConditionBtn(){
		...
		newC = createBtn(StaticRef.CREATE_BTN,SettPrivRef.SUPLR_RATE_P1_10);
		newC.addClickHandler(new NewSuplrTariffCondAction(groupTable, null, this));
		
		savC = createBtn(StaticRef.SAVE_BTN,SettPrivRef.SUPLR_RATE_P1_11);
		...
		delC = createBtn(StaticRef.DELETE_BTN,SettPrivRef.SUPLR_RATE_P1_12);
		delC.addClickHandler(new DeleteAction(groupTable));
		
		canC = createBtn(StaticRef.CANCEL_BTN,SettPrivRef.SUPLR_RATE_P1_13);
		...
		add_cond_map.put(SettPrivRef.SUPLR_RATE_P1_10, newC);
		del_cond_map.put(SettPrivRef.SUPLR_RATE_P1_12, delC);
		sav_cond_map.put(SettPrivRef.SUPLR_RATE_P1_11, savC);
		sav_cond_map.put(SettPrivRef.SUPLR_RATE_P1_13, canC);
...
//		txt_global.setEndRow(true);
		...
		TFF_TYP.setDisabled(true);
		//TFF_TYP.setDefaultValue("55CDDBECF2BF49D1A451F09A263F3F47");  //供应商
...
//		Util.initComboValue(OBJECT_ID, "BAS_SUPPLIER", "ID", "SHORT_NAME"," WHERE ENABLE_FLAG = 'Y'", "");
		...
		initSUPPLIERByQuery(OBJECT_NAME, OBJECT_ID);
		...
		TextItem COM_FLAG = new TextItem("COM_FLAG");
		COM_FLAG.setValue("N");
		COM_FLAG.setVisible(false);
		
		form.setItems(COM_FLAG,TFF_TYP,OBJECT_ID,OBJECT_NAME,START_DATE,END_DATE,DOC_TYP,EXEC_ORG_ID_NAME,C_ORG_FLAG, chk_enable,EXEC_ORG_ID,TFF_FLAG);
		return form;
	...
	private ToolStrip createDiscountBtn(){
		...
		newD = createBtn(StaticRef.CREATE_BTN,SettPrivRef.SUPLR_RATE_P1_10);
		newD.addClickHandler(new NewTariffSuplrDiscountAction(discountTable, null, this));

		
		savD = createBtn(StaticRef.SAVE_BTN,SettPrivRef.SUPLR_RATE_P1_11);
		...
		delD = createBtn(StaticRef.DELETE_BTN,SettPrivRef.SUPLR_RATE_P1_12);
		delD.addClickHandler(new DeleteAction(discountTable));
		
		canD = createBtn(StaticRef.CANCEL_BTN,SettPrivRef.SUPLR_RATE_P1_13);
		...
		add_discount_map.put(SettPrivRef.SUPLR_RATE_P1_10, newD);
		del_discount_map.put(SettPrivRef.SUPLR_RATE_P1_12, delD);
		sav_discount_map.put(SettPrivRef.SUPLR_RATE_P1_11, savD);
		sav_discount_map.put(SettPrivRef.SUPLR_RATE_P1_13, canD);
...
//		detail_ck_map.put("CARRY_TYP", StaticRef.CHK_NOTNULL + Util.TI18N.CARRY_TYP());
		...
		cache_map.put("TFF_TYP", "55CDDBECF2BF49D1A451F09A263F3F47");  //供应商
		...
		fee_cache_map.put("FEE_ATTR","E43845662CE04B80995D3AE8FB41D11F");  //应付费用
	...
	public SuplrRateManagerView getThis(){
		return this;
	}

	@Override
	public Canvas createCanvas(String id,TabSet tabSet) {
		SuplrRateManagerView view = new SuplrRateManagerView();
		view.setFUNCID(id);
	...
	private void initSUPPLIERByQuery(final ComboBoxItem OBJECTNAME, final TextItem OBJECTID) {
		DataSource custDS = SUPPLIERDS.getInstance("VC_SUPPLIER");
		ListGridField CUSTOMER_CNAME = new ListGridField("SUPLR_CNAME",Util.TI18N.CUSTOMER_NAME());
		ListGridField CUSTOMER_CODE = new ListGridField("SUPLR_CODE",Util.TI18N.CUSTOMER_CODE(),80);
		OBJECTNAME.setOptionDataSource(custDS);  
		OBJECTNAME.setDisabled(false);
		OBJECTNAME.setShowDisabled(false);
		OBJECTNAME.setDisplayField("FULL_INDEX"); 
		OBJECTNAME.setPickListBaseStyle("myBoxedGridCell");
		OBJECTNAME.setPickListWidth(230);
	
		Criteria criteria = new Criteria();
		criteria.addCriteria("OP_FLAG","M");
		OBJECTNAME.setPickListCriteria(criteria);
		
		OBJECTNAME.setPickListFields(CUSTOMER_CODE, CUSTOMER_CNAME);
		OBJECTNAME.addChangedHandler(new com.smartgwt.client.widgets.form.fields.events.ChangedHandler() {
			
			@Override
			public void onChanged(
					com.smartgwt.client.widgets.form.fields.events.ChangedEvent event) {
				final Record selectedRecord  = OBJECTNAME.getSelectedRecord();
				if(selectedRecord != null){
					OBJECTNAME.setValue(selectedRecord.getAttribute("SUPLR_CNAME"));
					OBJECTID.setValue(selectedRecord.getAttribute("ID"));
				}
				
			}
		});
		
		OBJECTNAME.addBlurHandler(new BlurHandler() {
			
			@Override
			public void onBlur(BlurEvent event) {
				if(!ObjUtil.isNotNull(OBJECTNAME.getValue()) && OBJECTID != null){
					OBJECTID.setValue("");
				}
			}
		});
