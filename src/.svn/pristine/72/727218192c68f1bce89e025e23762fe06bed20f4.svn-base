...
//        Util.initCustComboValue(CUSTOMER_NAME, vm, "");
        ...
        SGText CONT_EMAIL = new SGText("CONT_EMAIL","电邮");
...
//		ZIP.setLength(3);
        ...
        mainForm.setItems(CUSTOMER_ID,CUSTOMER_NAME,ADDR_CODE,ENABLE,ADDR_NAME,HINT_CODE,EXEC_ORG_ID_NAME,AREA_ID,AREA_ID2,AREA_ID3,AREA_NAME,AREA_NAME2,AREA_NAME3,ADDRESS,
        		CONT_NAME,CONT_TEL,CONT_EMAIL,ADDR_TYP,LONGITUDE,LATITUDE,APPOINT_FLAG,APPOINT_HOURS,GRADE,LOAD_FLAG,RECV_FLAG,TRANSFER_FLAG,notes,CLASS_FLAG);
...
//        toolStrip.addSeparator();
        ...
        IButton inputButton = createBtn(StaticRef.IMPORT_BTN, BasPrivRef.ADDR_P0_06);
		inputButton.addClickHandler(new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				if(uploadWin == null){
					
					JavaScriptObject jsobject = sectionStack.getSection(0).getAttributeAsJavaScriptObject("controls");
					Canvas[] canvas = null;
					...
					uploadWin = new UploadFileWin().getViewPanel("address.xls","TMP_ADDRESS_IMPORT","SP_IMPORT_ADDRESS",table,pageForm);
				}else{
					uploadWin.show();
				}
			}
...
//		EXEC_ORG_ID_NAME.setStartRow(true);
        ...
        form.setItems(ADDR_CODE,ADDR_NAME,enable,ADDR_TYPE,AREA_ID,NULL_AREA_FLAG,CUSTOMER_ID,CUSTOMER_NAME,
