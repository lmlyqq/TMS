...
/**
 * 运输管理--原始订单--右键--审核操作（审核完成后自动按温层拆分，生成多个托运单）
 * @author fanglm
 *
 */
public class SFOdrAuditAction implements ClickHandler,com.smartgwt.client.widgets.events.ClickHandler{
	...
	public SFOdrAuditAction(ListGrid table,SFOrderView view){
		this.table = table;
		this.view = view;
	}
	...
	private void doSome() {
		...
		//存储过程名称
		String proName="SP_SFORDER_AUDIT(?,?,?)"; //odr_no,user_id,output_result
		...
		if(sf.toString().length() > 0){
			SC.warn("订单 "+sf.substring(0,sf.length()-1) +",非【" + StaticRef.SFODR_STATUS_CONFIRM_NAME + "】状态, 不能执行审核操作！");
			return;
		}
