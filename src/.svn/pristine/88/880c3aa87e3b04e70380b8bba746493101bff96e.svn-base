...
public class DeletePayReqAction implements ClickHandler,com.smartgwt.client.widgets.form.fields.events.ClickHandler {

	private DynamicForm form;
	private ListGrid table;
	private ListGridRecord[] records;
	public DeletePayReqAction(ListGrid p_table, DynamicForm p_form) {
		table = p_table;
		form = p_form;
	}
    	...
    	Util.async.doDelete(descrList, sqlList, new AsyncCallback<String>() {
			...
			public void onSuccess(String result) {
				if(result.equals(StaticRef.SUCCESS_CODE)) {
					...
					String sql1="delete from  BILL_PAY_REQDETAILS where REQ_NO='"+table.getSelectedRecord().getAttribute("REQ_NO")+"'";
					list.add(sql1);
					String sql2="delete from BILL_PAY_INVOICEINFO where REQ_NO='"+table.getSelectedRecord().getAttribute("REQ_NO")+"'";
					list.add(sql2);
					String sql3="delete from BILL_PAY_PAYLOG where REQ_NO='"+table.getSelectedRecord().getAttribute("REQ_NO")+"'";
					list.add(sql3);
					String sql4="update bill_pay_initdetails set INVOICE_STAT = '10' where init_no ='"+table.getSelectedRecord().getAttribute("INIT_NO")+"'";
					list.add(sql4);
					String sql5="update bill_pay_initial set PAY_FLAG = 'N' where init_no ='"+table.getSelectedRecord().getAttribute("INIT_NO")+"'";
					list.add(sql5);
