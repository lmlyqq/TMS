...
public class DeleteInvoGrpAction implements ClickHandler,com.smartgwt.client.widgets.form.fields.events.ClickHandler {

	private DynamicForm form;
	private ListGrid table;
	private ListGridRecord[] records;
	public DeleteInvoGrpAction(ListGrid p_table, DynamicForm p_form) {
		table = p_table;
		form = p_form;
	}
	...
	private void doDelete() {
    	...
    	for(int i = 0; i < records.length; i++) {
    		...
    		sf = new StringBuffer();
    		sf.append("update bill_settle_info set invo_grp_id='' where invo_grp_id='");
    		sf.append(records[i].getAttribute("INVO_GRP_ID"));
    		sf.append("'");
    		sqlList.add(sf.toString());
    	}
