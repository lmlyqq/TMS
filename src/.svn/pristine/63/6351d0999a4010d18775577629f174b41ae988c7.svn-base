...
public class NewDmgInvoiceAction implements ClickHandler,com.smartgwt.client.widgets.form.fields.events.ClickHandler{
	...
	public NewDmgInvoiceAction(DynamicForm p_form, Map<String, String> map) {
		form = p_form;
		initMap = map;
	}

	public NewDmgInvoiceAction(DynamicForm p_form, SGTable sgtable,RecDamageView view) {
		form = p_form;
		table=sgtable;
		this.view=view;
	}
	...
	public void click(){
		...
		if(form != null){
			...
			if(record!=null){
			String BUSS_ID=record.getAttribute("CUSTOMER_ID");
			if(BUSS_ID!=null){
			...
			form.setValue("DAMAGE_NO", table.getSelectedRecord().getAttribute("DAMAGE_NO"));
