...
public class AuditAction implements ClickHandler{
	...
	public AuditAction (SGTable table,boolean isAud,SuplrFeeSettView view,String fee_typ){
		this.table = table;
		this.isAud = isAud;
		this.view = view;
...
//				}
	...
	private void doAsLoad(String loginUser){
		...
		for(int i=0;i<records.length;i++){
			if(Util.iff(records[i].getAttribute("ACCOUNT_STAT"),"10").equals("20")){
				MSGUtil.sayError("费用已确认对账，不允许修改提交!");
				return;
			}
				...
				if(StaticRef.SUCCESS_CODE.equals(result.substring(0,2))){
					...
					for(int i=0;i<records.length;i++){
						if(isAud){
							...
							records[i].setAttribute("AUDIT_STAT_NAME", "审批中");
							view.initLoadFeeBtn(5);
						}else{
							records[i].setAttribute("AUDIT_STAT", NO_AUDIT);
							records[i].setAttribute("AUDIT_STAT_NAME", "待审批");
...
//	}
	...
	private void doAsOrder(String loginUser){
		int[] record = cView.table.getAllEditRows();
		String n="";
		for(int j=0;j<record.length;j++){
			n=n+record[j]+",";
		}
		ListGridRecord[] records = cView.table.getRecords();
		ArrayList<String> sqlList = new ArrayList<String>();
		StringBuffer sql = new StringBuffer();
		for(int i = 0; i < records.length; i++) {
			Record rec = records[i];
			if(n.contains(i+",")){
				if(cView.table.getEditedRecord(i).getAttribute("USE_FLAG").equals("true")){
					sql = new StringBuffer();
