 ...
 *
 */
public class LoadJob_Shpm_DS extends DataSource{
	private static LoadJob_Shpm_DS instance = null;

	public static LoadJob_Shpm_DS getInstance(String id, String tableName) {
		if (instance == null) {
			instance = new LoadJob_Shpm_DS(id, tableName);
		}
		return instance;
	}

	public static LoadJob_Shpm_DS getInstance(String id) {
		if (instance == null) {
			instance = new LoadJob_Shpm_DS(id, id);
		}
		return instance;
	}

	public LoadJob_Shpm_DS(String id, String tableName) {
...
//		DataSourceTextField ARRIVE_WHSE_TIME = new DataSourceTextField("ARRIVE_WHSE_TIME", Util.TI18N.ARRIVE_WHSE_TIME(),50);//调度单号
		...
		setFields(keyField, LOAD_NO,SHPM_NO,PLATE_NO,LOAD_WHSE_NAME,LOAD_STATUS_NAME,LOAD_NAME,VEHICLE_TYP_NAME,UNLOAD_NAME,TOT_QNTY,SUPLR_NAME);
