...
public class ConfirmDamageAction implements ClickHandler{
	...
	private String payProcName = "SP_DAMAGE_CONFIRM(?,?,?)";
	...
	public void onClick(ClickEvent event) {
		SC.confirm("确认后，系统会自动产生客户和承运商的货损货差扣款单，确认继续?", new BooleanCallback() {
			public void execute(Boolean value) {
                if (value != null && value) {
                	...
                	for(int i = 0; i < records.length; i++) {
                		...
                		pay_amount += Double.parseDouble(ObjUtil.ifNull(records[i].getAttribute("DRIVER_ACOUNT"),"0"));
                		notes = records[i].getAttribute("REASON");
                	}
                	System.out.println(rec_amount);
...
//            				if(result.substring(0,2).equals(StaticRef.SUCCESS_CODE)){
            		...
            		Util.async.execProcedure(getPayList(selRecord), payProcName, new AsyncCallback<String>() {
            			@Override
...
//		list.add(null);
//		list.add(selRecord.getAttribute("ODR_NO"));
...
//		return list;
//	}
//  ListGridRecord selRecord,double pay_amount,String notes	
	private ArrayList<String> getPayList(ListGridRecord selRecord){
