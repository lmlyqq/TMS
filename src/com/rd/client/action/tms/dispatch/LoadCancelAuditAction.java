 ...
 *
 */
public class LoadCancelAuditAction implements ClickHandler {

	private SGTable loadTable;
	public LoadCancelAuditAction(SGTable p_loadTable) {
	...
	public void onClick(MenuItemClickEvent event) {
		...
		if(records != null && records.length > 0) {
			...
			for(int i = 0; i < records.length; i++) {
				/*if(!records[i].getAttribute("DISPATCH_STAT_NAME").equals(StaticRef.AUDITED_NAME)) {
					SC.warn("调度单[" + records[i].getAttribute("LOAD_NO") + "]配车状态为[" + records[i].getAttribute("DISPATCH_STAT_NAME") + "],不能执行取消审核!");
					return;
				}*/
				if(!records[i].getAttribute("STATUS_NAME").equals(StaticRef.TRANS_CREATE_NAME)) {
					SC.warn("调度单[" + records[i].getAttribute("LOAD_NO") + "]非已创建状态,不能执行取消审核!");
					return;
				}
			...
			Util.async.execProcedure(json, "SP_LOADNO_CANCELAUDIT(?,?,?)", new AsyncCallback<String>() {
				@Override
