...
public class ChangedGrossWAction implements EditorExitHandler {

	private SGTable table;
	private DispatchView view;
	public ChangedGrossWAction(SGTable p_table, DispatchView p_view) {
		this.table = p_table;
		this.view = p_view;
	}
	@Override
	public void onEditorExit(EditorExitEvent event) {
		if(event != null) {
			...
			if(ObjUtil.isNotNull(event.getNewValue())) {
				double cur_gw = Double.parseDouble(ObjUtil.ifObjNull(event.getNewValue(),0).toString());
				double init_gw = Double.parseDouble(rec.getAttribute("G_WGT").toString());
				double rate = getRate(cur_gw, init_gw);
				if(cur_gw <= 0) {
