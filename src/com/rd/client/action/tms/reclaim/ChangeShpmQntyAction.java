 ...
 *
 */
public class ChangeShpmQntyAction implements EditorExitHandler {
	private SGTable table;
	
	public ChangeShpmQntyAction(SGTable table){
		this.table = table;
	}
...
//			    double UNLD_NWGT = Double.parseDouble(ObjUtil.ifObjNull(rec.getAttribute("UNLD_GWGT"),"0").toString());
//			    double UNLD_WORTH = Double.parseDouble(ObjUtil.ifObjNull(rec.getAttribute("UNLD_WORTH"),"0").toString());
				double LD_QNTY = Double.parseDouble(ObjUtil.ifObjNull(rec.getAttribute("LD_QNTY"),"0").toString());
//			    double UNLD_QNTY = Double.parseDouble(ObjUtil.ifObjNull(rec.getAttribute("UNLD_QNTY"), "0").toString());
				table.setEditValue(row, "VOL", rate*vol);
				table.setEditValue(row, "G_WGT", rate*g_wgt);
				table.setEditValue(row, "LOSDAM_QNTY", LD_QNTY-cur_qnty);
				table.setEditValue(row, "UNLD_QNTY_EACH", QNTY_EACH/QNTY*cur_qnty);
