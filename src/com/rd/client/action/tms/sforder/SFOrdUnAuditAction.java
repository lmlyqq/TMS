 ...
 *
 */
public class SFOrdUnAuditAction implements ClickHandler,com.smartgwt.client.widgets.events.ClickHandler{
	...
	public SFOrdUnAuditAction(ListGrid table,ValuesManager vm,SFOrderView view){
		this.table = table;
		this.vm = vm;
		this.view = view;
	}
	...
	protected void doSome(){
		...
		for(int i=0;i<list.length;i++){
			if(!StaticRef.SFODR_STATUS_AUDIT.equals(list[i].getAttribute("STATUS"))){
			...
			public void onSuccess(String result) {
				...
				for(int i=0;i<list.length;i++){
					if(errorId.indexOf(list[i].getAttributeAsString("ODR_NO")) >= 0){ //wangjun 2010-07-19
						list[i].setAttribute("STATUS", StaticRef.SFODR_STATUS_AUDIT);
						list[i].setAttribute("STATUS_NAME", StaticRef.SFODR_STATUS_AUDIT_NAME);
						table.updateData(list[i]);
					}
				}
				//FANGLM 2010-12-23 form刷新
				vm.setValue("STATUS", StaticRef.SFODR_STATUS_AUDIT);
				table.redraw();
