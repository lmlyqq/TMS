...
public class SaveReceRuleAction implements ClickHandler{
	...
	public SaveReceRuleAction(ListGrid p_table, ValuesManager p_form, HashMap<String, String> p_map, ReceRuleView view) {
		this.view = view;
		table = p_table;
		form = p_form;  
		map = p_map;
				...
				if(result.substring(0,2).equals(StaticRef.SUCCESS_CODE)) {
					...
					for(int i=0;i<num;i++){
						...
						if(view.basInfo.getItem("ATTR_VAL"+Integer.toString(i+1)).getValue()==null){
							...
							}else if(view.basInfo.getItem("OPER_OBJ"+Integer.toString(i+1)).getValue().toString().equals("目的城市")){
								sf.append("'"+ObjUtil.ifNull(view.basInfo.getItem("AREA_ID"+Integer.toString(i+1)).getValue().toString(),"")+"',");
							}else{
					...
					Util.async.excuteSQLList(sqlList, new AsyncCallback<String>() {
						...
						public void onSuccess(String result) {
							if(StaticRef.SUCCESS_CODE.equals(result.substring(0,2))){
								...
								cri.addCriteria("TFF_TYP","42666CA2DE904F6687FC172138CF3E51");
