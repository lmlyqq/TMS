...
public class NewUserSystemAction implements ClickHandler,com.smartgwt.client.widgets.menu.events.ClickHandler{
	...
	public NewUserSystemAction(SGTable p_table,SGTable usertable,Map<String, String> initMap,SGForm view) {
		table = p_table;
			...
			for(int i = 0; i < iter.length; i++) {
				...
				if(key.indexOf("CUSTOMER_ID") >= 0 ){
					...
					if(value.equals("Y") || value.equals("N")) {
						table.setEditValue(table.getRecords().length,"USER_ID",usertable.getSelectedRecord().getAttribute("USER_ID"));
