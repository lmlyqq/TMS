...
public class SavePersonAction implements ClickHandler{
	...
	public SavePersonAction(ListGrid p_table, ValuesManager p_form, HashMap<String, String> p_map, SGForm view) {
		table = p_table;
		form = p_form;  
		map = p_map;
		this.view = view;
		...
		if(record != null) {
			...
			if(obj != null && obj.size() > 1) {
				...
				if(result.equals(StaticRef.SUCCESS_CODE)) {
					if(form.getValueAsString("MOBILE")!=null){
						if(form.getValueAsString("MOBILE").length()!=11){
							SC.say("手机号码必须为11位");
							return;
						}
					}
