  		...
  		QNTY.addEditorExitHandler(new EditorExitHandler() {
			...
			public void onEditorExit(EditorExitEvent event) {
				if(event != null) {
					...
					ListGridRecord rec = skuTable.getRecords()[row];
					if(ObjUtil.isNotNull(event.getNewValue())) {
						...
						if(cur_qnty <= 0) {
							...
							skuTable.setEditValue(row, "QNTY", rec.getAttribute("QNTY"));
							skuTable.setEditValue(row, "VOL", rec.getAttribute("VOL"));
							skuTable.setEditValue(row, "G_WGT", rec.getAttribute("G_WGT"));
						...
						if(cur_qnty > 0 && rate > 0) {
							...
							skuTable.setEditValue(row, "VOL", rate*vol);
							skuTable.setEditValue(row, "G_WGT", rate*g_wgt);
