 ...
 */
@ClassForNameAble
public class TmsShipmentCompView extends SGForm implements PanelFactory {
...
//	private ListGridRecord[] selecteds;
	...
	private CheckboxItem isBizCode;

	/*public TmsShipmentCompView(String id) {
	    super(id);
	}*/
...
//				    }
		...
		isBizCode = new CheckboxItem("IS_BIZ_CODE", "大网过滤");
		isBizCode.setTitleOrientation(TitleOrientation.LEFT);
		isBizCode.setHeight(expButton.getHeight());
		isBizCode.setValue(true);
		isBizCode.setWidth(50);
		
		toolStrip.setMembersMargin(5);
		toolStrip.setMembers(searchButton, saveButton, execButton, expButton);
		toolStrip.addFormItem(isBizCode);
...
//        CUSTOMER_NAME.setHidden(true);
        ...
        ListGridField BIZ_CODE = new ListGridField("BIZ_CODE", "网点代码");
		BIZ_CODE.setCanEdit(false);
		BIZ_CODE.setWidth(80);
		...
		table.setFields(SHPM_NO,REFENENCE1,CUSTOMER_NAME,LOAD_AREA_NAME2,UNLOAD_ADDRESS,BIZ_CODE, UNLOAD_ID,UNLOAD_NAME,UDF5,UNLOAD_AREA_ID2,ABNOMAL_STAT,ABNOMAL_NOTES);
		...
		table.addRowMouseDownHandler(new RowMouseDownHandler() {
			...
			public void onRowMouseDown(RowMouseDownEvent event) {
				...
				r = r == null ? table.getSelectedRecord() : r;
				if(r != null){
					initComboBoxItem(r, table);
				}
			}
		});
...
//				}
	...
	private void initComboBoxItem(Record r, final SGTable table){
		...
		String BIZ_CODE = r.getAttribute("BIZ_CODE");
		...
		criteria.addCriteria("ADDR_TYP","383297659DAA4C3CB084C9AB93A11F73,9E0057747CC24A4BB998749752D2C42E");
		criteria.addCriteria("AREA_ID2",UNLOAD_AREA_ID2);
		if(ObjUtil.isNotNull(BIZ_CODE) && 
				"true".equals(ObjUtil.ifObjNull(isBizCode.getValue(), "").toString())){
			criteria.addCriteria("BIZ_CODE",BIZ_CODE);
		}
			...
			public void onSuccess(String result) {
				...
				if(edits.length > 0) {
					...
					Util.async.execProcedure(json, "SP_SHPM_MODIFY(?,?,?,?,?)", new AsyncCallback<String>() {
						...
						@Override
						public void onSuccess(String result) {
							if(result.equals(StaticRef.SUCCESS_CODE) || result.startsWith("02")) {
								RefreshRecord();
								if(result.startsWith(StaticRef.SUCCESS_CODE)){
...
//	    }
	...
	public Canvas createCanvas(String id,TabSet tabSet) {
		TmsShipmentCompView view = new TmsShipmentCompView();
		view.setFUNCID(id);
