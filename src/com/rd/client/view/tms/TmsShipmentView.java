...
import com.rd.client.action.tms.CloseShpmHeadAction;
import com.rd.client.action.tms.PayoutShpmentWin;
...
import com.rd.client.action.tms.dispatch.ShpmSplitActionWin;
 ...
 */
@ClassForNameAble
public class TmsShipmentView extends SGForm implements PanelFactory {
	...
	/*public TmsShipmentView(String id) {
	    super(id);
	}*/
...
//				    }
		...
		//派发
		payoutButton = createBtn(StaticRef.PAYOUT_BTN,TrsPrivRef.SHPMASSIGN_P1_01);
//		payoutButton.addClickHandler(new PayoutShpmentWin(headTable,true));
		...
		payoutButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				if(headTable.getSelection().length > 0){
				     new PayoutShpmentWin(headTable, true,getThis()).getViewPanel().show();
		...
		closeButton = createBtn(StaticRef.CLOSE_BTN,TrsPrivRef.SHPMASSIGN_P1_04);
		...
		closeButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				//if(record.length > 0 && record != null && record.toString().trim().length()>0){
					new CloseShpmHeadAction(headTable,record,getThis()).getviewPanel().show();
...
//					payoutButton.enable();
					...
					//setButtonEnabled(TrsPrivRef.SHPMASSIGN_P1_03,freeButton,false);//wangjun 2011-3-8
					//setButtonEnabled(TrsPrivRef.SHPMASSIGN_P1_02,freezeButton,true);
					setButtonEnabled(TrsPrivRef.SHPMASSIGN_P1_04,closeButton,true);
					setButtonEnabled(TrsPrivRef.SHPMASSIGN_P1_01,payoutButton,true);
...
//					payoutButton.disable();
					...
					//setButtonEnabled(TrsPrivRef.SHPMASSIGN_P1_03,freeButton,true);//wangjun 2011-3-8
					//setButtonEnabled(TrsPrivRef.SHPMASSIGN_P1_02,freezeButton,false);
					setButtonEnabled(TrsPrivRef.SHPMASSIGN_P1_04,closeButton,false);
					setButtonEnabled(TrsPrivRef.SHPMASSIGN_P1_01,payoutButton,false);
				}
				else {
//					freeButton.disable();
//					freezeButton.disable();
...
//	    getQuickKey();
	    ...
	    if(isPrivilege(TrsPrivRef.SHPMASSIGN_P1_10)) {                         //wangjun 2011-5-3
	    	MenuItem split = new MenuItem("货量拆分",StaticRef.ICON_CONFIRM);
	    	split.addClickHandler(new com.smartgwt.client.widgets.menu.events.ClickHandler() {
				...
				public void onClick(MenuItemClickEvent event) {
					...
					if(detailTable != null && detailTable.getSelection().length > 0){
						if(isSplitValid()) {
							new ShpmSplitActionWin(detailTable,headTable,getThis()).getViewPanel().show();
						}
	    	...
	    	refreshTableAction = new RefreshshpmTableAction(headTable, getThis());
	    }
	    if(isPrivilege(TrsPrivRef.SHPMASSIGN_P1_11)) {
	    ...
	    if(isPrivilege(TrsPrivRef.SHPMASSIGN_P1_12)) {                         //wangjun 2011-5-3
	    ...
	    if(isPrivilege(TrsPrivRef.SHPMASSIGN_P1_13)) {
	    ...
	    /*if(isPrivilege(TrsPrivRef.SHPMASSIGN_P1_16)) { 
	    ...
	    if(isPrivilege(TrsPrivRef.SHPMASSIGN_P1_17)) { 
	    ...
	    if(isPrivilege(TrsPrivRef.SHPMASSIGN_P1_14)) { 
	    ...
	    if(isPrivilege(TrsPrivRef.SHPMASSIGN_P1_15)) {
		    ...
		    inItem.addClickHandler(new com.smartgwt.client.widgets.menu.events.ClickHandler() {
				...
				public void onClick(MenuItemClickEvent event) {
					if(headTable != null && headTable.getSelection().length > 0){
						//弹出窗口
						new ModifyShpmWin(headTable, headTable.getSelection(),true);
...
//	    if(isPrivilege(TrsPrivRef.SHPMASSIGN_P1_10)) {
//	    	
//	    	menu.addItem(itemSeparator); 
...
//		    
//	    }
	    if(isPrivilege(TrsPrivRef.SHPMASSIGN_P1_05)) {                         //wangjun 2011-3-8
...
//	    menu.setItems(allSelect,unselect,sumInfo); 
	    ...
	    if(isPrivilege(TrsPrivRef.SHPMASSIGN_P1_09)) {
			MenuItem expList = new MenuItem("明细导出",StaticRef.ICON_EXPORT);  //wangjun 2011-4-2     明细导出
			menu.addItem(expList);
			expList.addClickHandler(new ExportShpmItemAction(headTable, ") order by addtime desc "));
	    }
	    
	    if(isPrivilege(TrsPrivRef.SHPMASSIGN_P1_18)) {
...
//		Util.initCodesComboValue(ASSIGN_STAT_NAME, "ASSIGN_STAT");
		...
		SGCombo ROUTE_ID = new SGCombo("ROUTE_ID", Util.TI18N.ROUTE_NAME());	
...
//        SGCheck UDF6 = new SGCheck("UDF6", "未补码");	
//		UDF6.setValue(false);
...
//		EMPTY_FLAG.setDefaultValue("Y");
	    ...
	    form.setItems(CUSTOMER_ID,CUSTOMER_NAME,ODR_NO,SHPM_NO,CUSTOM_ODR_NO,
			      LOAD_AREA_NAME,LOAD_AREA_ID,UNLOAD_AREA_NAME,UNLOAD_AREA_ID,REFENENCE1,SUPLR_NAME,
			      LOAD_NAME,LOAD_ID,UNLOAD_NAME,UNLOAD_ID,
			      STATUS_FROM,STATUS_TO,PLATE_NO,ASSIGN_STAT_NAME,BIZ_TYP,ODR_TIME_FROM,ODR_TIME_TO,TRANS_COND, 			     
				   ROUTE_ID,EXEC_ORG_ID,EXEC_ORG_ID_NAME,TRANS_SRVC_ID,ORD_ADDTIME_FROM,ORD_ADDTIME_TO,C_ORG_FLAG,EMPTY_FLAG,SHPM_FLAG);
		return form;
...
//		    return false;   
//		    }   
	public TmsShipmentView getThis(){
		return this;
	}
	...
	public Canvas createCanvas(String id,TabSet tabSet) {
		TmsShipmentView view = new TmsShipmentView();
		view.setFUNCID(id);
